...
const API_KEY = "YOUR API FROM A4F";
...
export const analyzeContentWithA4F = async (
  text: string,
  imageFile: File | null
  ...
  try {
    ...
    const textAnalysisPromise = (async () => {
      ...
      const data = await response.json();
      if (data.error)
        throw new Error(data.error.message || "Text Analysis API Error");
    ...
    const [textResult, imageResultString] = await Promise.all([
      textAnalysisPromise,
